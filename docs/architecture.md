...
Staging has a fixed evaluation revision and a server-side quarter-hour Cron trigger. One invocation computes and stores the deterministic 90-day baseline; a later invocation leases and runs the one-call assisted season. A stale `baseline-running` lease may retry because it cannot spend money. `assisted-running`, `failed`, and `complete` are non-retrying states, so a killed paid phase cannot buy an unknown answer twice. Retrying that phase deliberately requires a new revision. Production declares no evaluation trigger. The public staging API can read the report but cannot trigger, reset, or configure paid calls. Because Worker secrets are environment-specific, staging requires its own `DEEPSEEK_API_KEY`; post-deploy smoke verification requires `modelReady: true`.

The staging deployment polls that read-only report for up to 45 minutes, enough for two quarter-hour phases. Terminal failure or a stranded paid lease makes the deployment red; completion prints the bounded report into the GitHub Actions record. This is observation, not a second trigger, so deployment retries cannot initiate extra provider calls for the same revision.
