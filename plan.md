...
- DeepSeek's authority is now choice-only: `{obligationId, choice, note}`. Deterministic code composes Sal's full legal day from that choice, so a successful model call cannot erase ordinary work, meals, social intent, or rest.
